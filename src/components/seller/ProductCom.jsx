...
function ProductCom({
  ...
  return (
    <div className="flex h-full md:px-4 px-2 py-3 gap-4 bg-gray-100 rounded-lg dark:bg-[#0f0f0f] dark:text-gray-200 transition-all duration-300">
      <Link
        href={`/product/${productId}`}
        className="rounded-lg overflow-hidden min-w-[120px] min-h-[157px] sm:hidden block"
      >
        <Image
          src={image}
          width={120}
      ...
      <Link
        href={`/product/${productId}`}
        className="rounded-lg overflow-hidden min-w-[180px] min-h-[157px] hidden sm:block"
      >
        <Image
          src={image}
          width={180}
